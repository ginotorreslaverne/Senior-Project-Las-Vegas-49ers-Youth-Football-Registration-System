...
Can view, access, edit player information, including division, remove players, and create a CSV file for Offline/Coaches Use.
*/
// -----------------------------------------------------------------------------------------------------------------------
#include <iostream>
#include <sqlite3.h>
...
void exportPlayersToCSV(sqlite3 *db);
bool login();
...
// Function to export players to CSV File
void exportPlayersToCSV(sqlite3 *db) {
    string query = "SELECT ROWID, Name, DOB, Guardian, Phone, Email, Division FROM Players;";
    sqlite3_stmt *stmt;
    
    if (sqlite3_prepare_v2(db, query.c_str(), -1, &stmt, nullptr) != SQLITE_OK) {
        cerr << "Error preparing export query: " << sqlite3_errmsg(db) << endl;
        return;
    }
    
    ofstream outFile("players_export.csv");
    if (!outFile.is_open()) {
        cerr << "Failed to open file for export.\n";
        sqlite3_finalize(stmt);
        return;
    }
    
    // Write CSV headers
    outFile << "ID,Name,DOB,Guardian,Phone,Email,Division\n";
    ...
    cout << "Export complete! Data saved to 'players_export.csv'\n";
} 