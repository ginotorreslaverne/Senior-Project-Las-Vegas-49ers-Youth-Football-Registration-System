...
// Purpose: 
/* 
Backend Development for Senior Project: 
User Input Handling: Collect and validate player details. (Added ID Assignment)
...
void initializeDatabase(sqlite3*& db) {
    sqlite3_open("registration.db", &db);
...
void insertPlayerIntoDB(sqlite3* db, const Player& p) {
    string query = "INSERT INTO Players (Name, DOB, Guardian, Phone, Email, Division) VALUES ('" +
                   p.name + "', '" + p.dob + "', '" + p.guardian + "', '" + p.phone + "', '" +
                   p.email + "', '" + p.division + "');";
...
void loadPlayersFromDB(sqlite3* db, vector<Player>& players) {
    ...
    if (sqlite3_prepare_v2(db, query.c_str(), -1, &stmt, nullptr) == SQLITE_OK) {
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            ...
            p.dob = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 2));
            p.guardian = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 3));
            p.phone = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 4));
            p.email = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 5));
            p.division = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 6));
            players.push_back(p);
        }
    }
...
void displayPlayers(const vector<Player>& players) {
    ...
    for (const auto& p : players) {
        cout << "ID: " << p.id << " | Name: " << p.name << " | DOB: " << p.dob
             << " | Guardian: " << p.guardian << " | Phone: " << p.phone
             << " | Email: " << p.email << " | Division: " << p.division << "\n";
    }
}
